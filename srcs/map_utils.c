...
int check_zero_surroundings(char **tab, int x, int y)
{
    ...
    if (are_sorroundings_valid(c_up) && are_sorroundings_valid(c_down)
        ...
        && are_sorroundings_valid(c_top_left) && are_sorroundings_valid(c_top_right)
        && are_sorroundings_valid(c_bottom_left) && are_sorroundings_valid(c_bottom_right))
        return (1);
    return (0);
}
...
static void check_player_surroundings(t_data *data, char **map, int x, int y)
{
    int map_height = data->map->map_height;
